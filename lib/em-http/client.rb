...
module EventMachine
  ...
  class HttpClient < Connection
    ...
    def finished?
      p "========#{@state}======#{@state.class}====#{@bytes_remaining}"
      @state == :finished || @state == :chunk_header || (@state == :body && @bytes_remaining.nil?)
    end
    ...
    def unbind
      if finished? && (@last_effective_url != @uri) && (@redirects < @options[:redirects])
        begin
          ...
          new_http = reconnect(req.host, req.port, {:head => {
                  'Cookie' => "#{@response_header[SET_COOKIE].first.split(";")[0]}"
                  }})

          @response_header = HttpResponseHeader.new
          @state = :response_header
          @response = ''
          new_http.stream { |chunk| p chunk }
          @data.clear
    ...
    def parse_response_header
      ...
      if ''.respond_to?(:force_encoding) && /;\s*charset=\s*(.+?)\s*(;|$)/.match(response_header[CONTENT_TYPE].first)
